...
!     Local variables
      ...
      double precision :: G,e0,pt,K,Kb,nu,dsedee
      ...
      integer :: j
       ...
       K = pt*(1-2.d0*nu)/((1+nu)*G)*(1+e0)
       evol = sum(STRAN(1:3)+DSTRAN(1:3))
       edev(1:3) = STRAN(1:3)+DSTRAN(1:3) - evol/3.d0
       edev(4:6) = 0.5d0*(STRAN(4:6)+DSTRAN(4:6))

       stress = 2.d0*G*edev
       stress(1:3) = stress(1:3) + pt/3.d0*(1-EXP(-(1+e0)/K*evol))

       Kb = pt/3.d0*(1+e0)/K*EXP(-(1+e0)/K*evol)-2.d0*G/3.d0
