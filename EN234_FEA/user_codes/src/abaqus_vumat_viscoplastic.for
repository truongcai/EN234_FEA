...
!
       ...
       double precision E,xnu,Y,e0,m,edot0,alp,td,H,S,Om
       double precision f,dfde
       double precision eplas,deplas,ta,dta,C,sig0,c1,ds0
!
...
!
     ...
     1                      - sum(strainInc(k,1:ndir))/3.d0
        ...
        !Compute second part of sig_ij^(n+1)
...
!     Elastic increment (either no stress, or no plastic strain rate)
       ...
       do while (err>tol)
         ...
         !Computing delta ta
         dta = (ta*(exp(-deplas/Om)-1)+Om*dt/deplas)/
     1                    (1+Om/deplas*exp(-deplas/Om))

!         dta = dt-ta/Om*deplas
         !Computing C
         C = 1 - exp(-((ta+dta)/td)**alp)
         !Computing sig0
         sig0 = Y*(1+(eplas+deplas)/e0)**m
...
!         write(1,*) 'f',f
         !N-R for new deplas
         ddtade= ((-ta/Om*exp(-deplas/Om)-Om*dt*deplas**(-2))+
     1(ta*(exp(-deplas/Om)-1)+dt*Om*1/deplas)*1/deplas*exp(-deplas/Om))/
     2(1+Om/deplas*exp(-deplas/Om))
!         ddtade= -ta/Om
         ds0 = Y*m*(1.d0+(eplas+deplas)/e0)**(m-1.d0)*1.d0/e0
         dfde=-(ds0/S+H*alp*((ta+dta)/td)**(alp-1.d0)*ddtade/td*(1-C))
     &                   - 1.5d0*E/(S*(1.d0+xnu)) - 1/deplas
